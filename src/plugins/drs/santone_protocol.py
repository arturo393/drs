...
class SantoneProtocol(ComunicationProtocol):
    ...
    def _decode_ifboard_command(self) -> int:
        module_function_index = 1
        data_type_index = 3
        cmd_number_index = 4
        respond_flag_index = 5
        ...
        cmd_data_index = 7
        module_function = self._reply[module_function_index]
        data_type = self._reply[data_type_index]
        command_number = self._reply[cmd_number_index]

        if command_number == self._command_number.value:
            response_flag = self._reply[respond_flag_index]
            command_body_length = self._reply[cmd_body_length_index]
            command_body = self._reply[cmd_data_index:
                                       cmd_data_index + command_body_length]
            if response_flag == ResponseFlag.SUCCESS:
                self._reply_command_data = command_body
                self._message = self._decoder.ifboard_decode(
                    self._command_number, command_body)
                return 1
            else:
                return 0
        else:
