...
def analizar_argumentos():
    ...
    ap.add_argument("-hlcu", "--highLevelCriticalUL", required=False, help="highLevelCritical es requerido",
                    default=200)
    ap.add_argument("-hlwd", "--highLevelWarningDL", required=False, help="highLevelWarning es requerido", default=200)
    ap.add_argument("-hlcd", "--highLevelCriticalDL", required=False, help="highLevelCritical es requerido",
                    default=200)
    ap.add_argument("-hltw", "--highLevelWarningTemperature", required=False,
                    help="highLevelWarningTemperature es requerido", default=200)
    ap.add_argument("-hltc", "--highLevelCriticalTemperature", required=False,
...
def setRs485CmdFrames(args):
    ...
    frame_list.append(rs485.obtener_trama('set', 'dru', '00', '00', '094C0B' + mac + '170500' + sn, '22', '00',
                                          args['opt'] + args['dru']))
    frame_list.append(
        rs485.obtener_trama('query', 'dru', '00', '00', '04010500040305000406050004250500044004000441040004EF0B00',
                            '1e', '00', args['opt'] + args['dru']))
    frame_list.append(
        rs485.obtener_trama('query', 'dru', '00', '00', '07180A0000000007190A00000000071A0A00000000071B0A00000000',
                            '1e', '00', args['opt'] + args['dru']))
    frame_list.append(rs485.obtener_trama('query', 'dru', '00', '00',
                                          '0510040000051104000005120400000513040000051404000005150400000516040000051704000005180400000519040000051A040000051B040000051C040000051D040000051E040000051F040000',
                                          '52', '00', args['opt'] + args['dru']))
    return frame_list


def set_ltel_cmd_frame(args):
    frame_list = list()
    frame_list.append(
        rs485.obtener_trama('query', 'dru', '00', '00', '04010500040305000406050004250500044004000441040004EF0B00',
                            '1e', '00', args['opt'] + args['dru']))
...
def get_vswr_temperature_table(parameter_dic):
    ...
    table2 += "</thead>"
    table2 += "<tbody>"
    table2 += "<tr align=\"center\" style=font-size:12px><td>" + parameter_dic['paTemperature'] + " [°C]</td><td>" + \
...
def main():
    ...
    queries = set_ltel_cmd_frame(args)
    query_id = args['opt'] + args['dru']
    serial = rs485.setSerial('/dev/ttyS1', 19200)
    reply_time = time.time()

    replies = rs485.writeSerialQueries(queries, serial)
    if len(replies) == 0:
        sys.stderr.write(f"CRITICAL - No reply")
        sys.exit(CRITICAL)
