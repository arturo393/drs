...
def main():
    ...
    if Action == "query" or Action == "set":
        ...
        if Action == 'set':
            ...
            result_hex = a_bytearray.hex()
            try:
                result_ok = s8(int(result_hex, 16))
            except:
        ...
        if len(CmdNumber) > 4:
            ...
            hex_string = convertirRespuestaHumana(result_hex, Device, CmdNumber, HighLevelCritical, HighLevelWarning)

        if result_ok >= HighLevelCritical:
            print("CRITICAL Alert! - " + hex_string)
            sys.exit(CRITICAL)
        elif result_ok >= HighLevelWarning:
    ...
    try:
        ...
        if Device == 'dmu' and (CmdNumber == 'F8' or CmdNumber == 'F9' or CmdNumber == 'FA' or CmdNumber == 'FB'):
            ...
            while channel <= 16:
                ...
                Table += "<tr><td>" + str(channel).zfill(2) + "</td><td>" + texto[0:3] + "</td><td>" + texto[
                                                                                                       4:22 - 6 + 2] + "</td><td>" + texto[
                                                                                                                                     23:40 - 6 + 2] + "</td></tr>"
                channel += 1
                i += 8
            Table += "</tbody></table>"
            ...
            # convirtiendo hex to decimal
                ...
                CmdNumber == '0300' or CmdNumber == '0104' or CmdNumber == 'EF0B' or CmdNumber == '0102'
        ...
        # convirtiendo hex to decimal
                ...
                CmdNumber == '0600' or CmdNumber == '210B' or CmdNumber == '4004' or CmdNumber == '4104'
        ...
        # convirtiendo hex to decimal con signo
            ...
            tmp = str(formato) + list['unidad'] + "|" + list['variable'] + "=" + str(decSigned) + ";" + str(
                high_level_warning) + ";" + str(high_level_critical)
            Result = tmp
            ...
            Result = hex + " " + Table
                ...
                CmdNumber == '180A' or CmdNumber == '190A' or CmdNumber == '1A0A' or CmdNumber == '1B0A')):
            byte = Result
            byteInvertido = byte[6:8] + byte[4:6] + byte[2:4] + byte[0:2]
            hex_as_int = int(byteInvertido, 16)
            ...
            Result = str(formato) + dataDRU[CmdNumber]
                ...
                CmdNumber == '1004' or CmdNumber == '1104' or CmdNumber == '1204' or CmdNumber == '1304'
...
def read_serial_frame(s):
    ...
    while not is_data_ready and count > 0:
        ...
        if byte_received_counter == 0 and rcv_hex == '7e':
            rcv_hex_array.append(rcv_hex)
            hexadecimal_string = hexadecimal_string + rcv_hex
            byte_received_counter = byte_received_counter + 1
        elif byte_received_counter > 0 and rcv_hex_array[0] == '7e' and (
                byte_received_counter == 1 and rcv_hex == '7e') is not True:
            rcv_hex_array.append(rcv_hex)
            hexadecimal_string = hexadecimal_string + rcv_hex
            byte_received_counter = byte_received_counter + 1
            if rcv_hex == '7e' or rcv_hex == '7f':
                is_data_ready = True
            elif byte_received_counter > 100:
                return ""
        elif rcv_hex == '':
    ...
    hex_response = bytearray.fromhex(hexadecimal_string)
    return hex_response
...
def newBlankDmuParameter():
    ...
    RemoteQueryCommandNumberchannel = 1
    channel = 0
    while channel <= 16:
        parameters["channel" + str(channel) + "Status"] = "-"
        parameters["channel" + str(channel) + "ulFreq"] = "-"
