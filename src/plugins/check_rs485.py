...
def hastIdReplyError(reply, query_id):
    ...
    if (reply_id != query_id):
        logging.debug(
            "RU" + str(hex(query_id)) + " Error - reply id " + str(hex(reply_id)) + "is not the same " + str(reply))
        return 1
    return 0
...
def hasReplyError(reply, query_id):
    ...
    if (reply_crc != calculated_crc):
        ...
        reply = ''.join(format(x, '02x') for x in reply)
        logging.debug(
            query_id + " Error - CRC reply: " + str(reply_crc) + "  CRC calculated: " + str(calculated_crc) + " " + str(
                reply))
        return 1
    return 0
