...
def analizar_argumentos():
    ...
    if DmuDevice1 == "" and Device == 'dmu':
        sys.stderr.write("RS485 CRITICAL - error = El dmuDevice1 es obligatorio\n")      
        sys.exit(2)  

    if DmuDevice2 == "" and Device == 'dmu':
        sys.stderr.write("RS485 CRITICAL - error = El dmuDevice2 es obligatorio\n")      
        sys.exit(2)      
    ...
    if (CmdBodyLenght == "" and Action == 'set') or (CmdBodyLenght == "" and Device == 'dru'):
        sys.stderr.write("RS485 CRITICAL - error = cmdBodyLenght es obligatorio")      
        sys.exit(2)  
...
def obtener_trama(Action, Device, DmuDevice1, DmuDevice2, CmdNumber, CmdBodyLenght, CmdData, DruId):      
    ...
    if (Device == 'dru'): 
        ...
        try:
            ...
            sys.stderr.write("RS485 CRITICAL - error = CmdBodyLenght no tiene formato hexadecimal")      
            sys.exit(2)     
        ...
        if lenTramaLengthCodeData != cant_bytes: 
            sys.stderr.write("RS485 CRITICAL - error = CmdBodyLenght + CmdNumber + CmdData, no corresponde a la cantidad de bytes indicados\n")      
            sys.exit(2)  
        if (Action == 'set'):
...
def validar_trama_respuesta(hexResponse, Device):
    try:
        ...
        if (
                ...
                sys.stderr.write("RS485 CRITICAL - error = Error trama de salida invalida\n" )      
                sys.exit(2) 
        if Device == 'dru':
            #print('Entro aqui')
        ...
        sys.stderr.write("RS485 CRITICAL - error = Error al leer trama de salida\n")      
        sys.exit(2)      
...
def main():
    ...
    try:
        ...
        sys.stderr.write("RS485 CRITICAL - error = Error al abrir puerto %s \n" % str(Port))   
        sys.exit(1)
    ...
    if Action == "query" or Action == "set":
        ...
        while not isDataReady and rcvcount < 200:
            ...
            if(rcvHex == ''):
                isDataReady = True
                sys.stderr.write("RS485 CRITICAL - error = al leer el puerto de salida %s \n" % str(Port))   
                sys.exit(2)
        ...
        if Action == 'set':            
            if len(data) != 0 and Device == 'dmu':
                sys.stderr.write("RS485 CRITICAL - error = al escribir puerto dmu %s \n" % str(Port))   
                sys.exit(2)
            elif len(data) == 0 and Device == 'dru':
                sys.stderr.write("RS485 CRITICAL - error = al escribir puerto dru %s \n" % str(Port))   
                sys.exit(2)
            else:   
               if Device == 'dru':
