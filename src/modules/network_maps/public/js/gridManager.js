function drawGrid() {
  processData = (response) => {
    ...
    for (i = 0; i < hosts.length; i++) {
      if (hosts[i]['attrs'].state === 0) {
        ...
        hosts[i]['attrs'].state === 1 &&
        !hosts[i]['attrs'].last_reachable
      ) {
        font_size = 20;
        color_border = 'purple';
  ...
  function startListeners(network) {
    network.on('click', function (params) {
      ...
      if (params.nodes[0] != undefined) {
        ...
        location.href =
          './network_maps/module/statusGrid#!/' +
  ...
  function fullscreenMode(
    ...
    $('#hud-down').html(
      '<h1>' +
        hostsDown +
        calculatePercentage(hostsDown, hosts.length) +
        ' Hosts DOWN' +
        '</h1>'
    );
    $('#hud-unreachable').html(
      '<h1>' +
        hostsUnreachable +
        calculatePercentage(hostsUnreachable, hosts.length) +
        ' Hosts UNREACHABLE' +
        '</h1>'
    );
    $('#hud-up').html(
      '<h1>' +
        hostsUp +
        calculatePercentage(hostsUp, hosts.length) +
        ' Hosts UP' +
        '</h1>'
    );
  ...
  function calculateNumCols(numHosts) {
    let screenRatio =
      Math.round(
        ($('#grid-container').innerWidth() /
          $('#grid-container').innerHeight()) *
          10
      ) / 10;
